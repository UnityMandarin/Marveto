...
describe('public content contracts', () => {
  ...
  it('publishes exactly three honestly labeled studio concepts', () => {
    expect(projects.map((project) => project.title)).toEqual(['APEX', 'Serein', 'Forma']);
    expect(projects.map((project) => project.sector.split(' · ')[0])).toEqual(['Motorsport', 'Construction', 'Medical']);
